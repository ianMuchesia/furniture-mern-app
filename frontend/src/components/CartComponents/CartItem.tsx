import React from "react";
import { cartProductModel } from "../../@types/type";
import { AiOutlineMinus, AiOutlinePlus } from "react-icons/ai";
import { useAppDispatch, useAppSelector } from "../../hooks/reduxHooks";
import {
  addToCart,
  removeItemFromCart,
  removeSingleItemFromCart,
} from "../../store/cartSlice";
interface Props {
  item: cartProductModel;
}

const CartItem = ({ item }: Props) => {
  const dispatch = useAppDispatch();

  const handleAddToCart = () => {
    dispatch(addToCart(item));
  };

  const handleRemoveSingleItemFromCart = () => {
    dispatch(removeSingleItemFromCart(item._id));
  };

  const handleRemoveItemFromCart = () => {
    dispatch(removeItemFromCart(item._id));
  };

  return (
    <li className="flex items-center">
      <img
        src={item.imageUrl}
        alt=""
        className="object-cover w-24 h-24  md:h-32 md:w-32  rounded"
      />

      <div className="ml-4">
        <h3 className="text-xl text-gray-900">{item.name}</h3>

        <dl className="mt-0.5 space-y-px  text-gray-600">
          <div>
            <dt className="inline font-semibold">Quantity:</dt>
            <dd className="inline">{item.quantity}</dd>
          </div>

          <div>
            <dt className="inline font-semibold">Price:</dt>
            <dd className="inline">Ksh. {item.price.toLocaleString()}</dd>
          </div>
        </dl>
      </div>

      <div className="flex items-center justify-end flex-1 gap-2">
        <div className="bg-gray-200 px-2 flex justify-between gap-1 ">
          <button className="hover:scale-110 mr-2" onClick={handleAddToCart}>
            <AiOutlinePlus />
          </button>
          <h5 className="">{item.quantity}</h5>
          <button
            className="hover:scale-110 ml-1"
            onClick={handleRemoveSingleItemFromCart}
          >
            <AiOutlineMinus />
          </button>
        </div>

        <button
          className="text-gray-600 transition hover:text-red-600 "
          onClick={handleRemoveItemFromCart}
        >
          <span className="sr-only">Remove item</span>

          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="w-4 h-4"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
            />
          </svg>
        </button>
      </div>
    </li>
  );
};

export default CartItem;
